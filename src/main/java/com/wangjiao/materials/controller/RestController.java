...
public class RestController {
    ...
    private JSONArray getJsonList(List<Rest> rests){
        ...
        for(int i=0;i<rests.size();i++)
        {
            ...
            specNames.add(material.getSpec());
            categoryNames.add(material.getCategory());
            unit.add(material.getUnit());
            repositoryNames.add(repository.getRname());
        }
        JSONArray json = ListToJson.restToJson(rests,materialNames,repositoryNames,specNames,categoryNames,unit);
        return json;
    }
}
